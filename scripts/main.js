...
const renderEditButton = ({ name, id }) =>
  `<button class='list-item-button list-item-edit' arial-label='Editar ${name}' title='Editar ${name}' data-id="${id}" ><img src='./assets/editIcon.svg'/></button>`;
const renderRemoveButton = ({ name, id }) =>
  `<button class='list-item-button list-item-remove' data-id="${id}" arial-label='Remover ${name}' title='Remover ${name}'><img src='./assets/removeIcon.svg'/></button>`;
const renderListItem = ({ name, url, id }) =>
  `<li class="list-item">${renderRemoveButton({
    name,
    id,
  })}<a class="list-item-link" href=${url} target="_blank" aria-label=${name}></a><span class='list-item-text-inner'><span class="list-item-text">${name}</span>${renderEditButton(
    { name, id }
  )}</span></li>`;
const renderListContainer = ({ listItems, shouldUseExtraColumn }) =>
  `<ol class="list-content"${
    shouldUseExtraColumn ? ` style="--max-column-count: 4"` : ""
  }>${listItems}<ol>`;
...
function renderList(data) {
  ...
  if (data && data.length > 0) {
    ...
    listContainer.innerHTML = renderListContainer({
      listItems,
      shouldUseExtraColumn,
    });
...
function handleFormSubmit(action) {
  const button = document.querySelector(".join-form-button");
...
const handleProjectUpdate = (id) => async (body) => {
...
function toggleHome(on) {
  ...
  joinForm.reset();
  joinFormContainer.classList[on ? 'add' : 'remove']("hidden");

  listContainer.classList[!on ? 'add' : 'remove']("hidden");
}
...
function setFormTexts({ title, button }) {
  const titleElement = document.querySelector(
    ".join-form-container .page-subtitle"
  );
  const buttonElement = document.querySelector(".join-form-button");
